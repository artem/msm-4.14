...
static inline void free_handle(unsigned long handle)
{
	...
	*(unsigned long *)handle = 0;
	...
	/*
	 * We are freeing a foreign handle if we are here. So holding page lock
	 * is not enough, an object can be freed for another zhdr.
	 *
	 * False positives are *not* possible here though since _allocation_ of
	 * a foreign handle happens with both pages locked.
	 */
